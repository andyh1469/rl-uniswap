...
class UniswapLPEnv(gym.Env):
  def __init__(self, price_history_df, volume_df, liquidity_df, initial_cash, min_tick, max_tick, tick_spacing, token0_decimals, token1_decimals, feeWeight, debug_mode = False):
    ...
    self.observation_space = spaces.Box(np.concatenate([np.zeros(2*(self.NUM_ACTIVE_TICK_RANGES) + 1), self.MIN_PRICE*np.ones(self.ETH_PRICE_HISTORY_OFFSET + 1), np.zeros(2*self.NUM_ACTIVE_TICK_RANGES)]), np.concatenate([sys.maxsize*np.ones(2*(self.NUM_ACTIVE_TICK_RANGES) + 1), self.MAX_PRICE*np.ones(self.ETH_PRICE_HISTORY_OFFSET + 1), sys.maxsize*np.ones(self.NUM_ACTIVE_TICK_RANGES), np.ones(self.NUM_ACTIVE_TICK_RANGES)]))
  ...
  def updatePositions(self, cur_price):
    ...
    self.new_position_token0[mixed_tick_lower_idx] = self.liquidities_provided[mixed_tick_lower_idx]*(np.sqrt(p_b) - np.sqrt(cur_price))/(np.sqrt(cur_price)*np.sqrt(p_b))
  ...
  def reset(self):
    ...
    total_position = np.concatenate([self.position_token1, self.position_token0, np.array(self.position_cash).reshape(1,)])
  ...
  def _take_action(self, action):
    ...
    if (self.debug_mode):
      print('Current ETH Price: '+str(cur_price))
      print('Current Position Token0: ')
      print(self.position_token0)
      print('Current Position Token1: ')
      print(self.position_token1)
      print('Current Position Cash: ')
      print(self.position_cash)
      print('New Position Token0: ')
      print(self.new_position_token0)
      print('New Position Token1: ')
      print(self.new_position_token1)
      print('New Position Cash: ')
      print(self.new_position_cash)
  ...
  def _compute_reward(self):
    ...
    if self.current_step >= len(self.price_history_df.index) - 2:
      reward = (cur_price*np.sum(self.new_position_token0) + np.sum(self.new_position_token1) + self.new_position_cash + self.feeWeight*sum(self.fee_history)) - self.INITIAL_CASH
    else:
      reward = 0
